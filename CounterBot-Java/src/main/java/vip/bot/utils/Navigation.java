...
public class Navigation {
    ...
    public static void dodgeLeft(UT2004BotVIPController<UT2004Bot> bot, boolean doubleDodge) {
        ...
        bot.getMove().dodgeLeft(inFront, doubleDodge);
    }

    public static void dodgeRight(UT2004BotVIPController<UT2004Bot> bot, boolean doubleDodge) {
        AgentInfo info = bot.getInfo();
        if (info.getVelocity().isZero(0.1)) return;
        Location inFront = info.getLocation().add(info.getVelocity().asLocation().getNormalized().scale(100));
        bot.getMove().dodgeRight(inFront, doubleDodge);
    }
